...
def Test(model, dataloader, criterion, params):
    ...
    test_loss = 0
    with torch.no_grad():
        for src_batch, trg_batch in tqdm(dataloader):
...
def translator(model, dataloader, params):
    tokenizer = params['tokenizer']
    max_len = params['max_len']
    vocab_size = params['vocab_size']
...
def main():
    ...
    test_dataset = TranslationDataset(ecmData)  #############################
    ...
    # Step 5: 학습 결과 생성
    model.eval()
    test_loss = Test(model, test_DL, criterion, params)
    all_translations = translator(model, test_DL, params)
